...
with serial.Serial('COM6', 115200) as uart2:
    ...
    time_sec = [float(item)-float(time_vals[0]) for item in time_vals]
    time_sec = [float(item)/10000 for item in time_vals]
    ...
    slope = round((float(log_volts[200])-float(log_volts[0]))/(float(time_sec[200])-float(time_sec[0])), 4)/10    
